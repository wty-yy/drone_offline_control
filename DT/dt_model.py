...
class GPTConfig(Config):
  ...
  def __init__(self, n_token, max_timestep, **kwargs):
    ...
    self.pred_actions = np.sum(self.n_actions)
    for k, v in kwargs.items():
      setattr(self, k, v)
    assert self.n_embd % self.n_head == 0, "n_embd must be devided by n_head"
    assert self.n_embd % len(self.n_actions) == 0, "n_embd must be devided by n_actions dim"
...
class GPT(nn.Module):
  ...
  def __call__(self, s, a, rtg, timestep, train: bool):
    ...
    tmp = []
    for n_action, action in zip(cfg.n_actions, jnp.transpose(a, (2, 0, 1))):  # (B, l, 4)
      tmp.append(Embed(n_action, cfg.n_embd // len(cfg.n_actions))(action))  # (B, l) -> (B, l, N_e//4)
    a = jnp.concatenate(tmp, -1)  # (B, l, N_e//4) * 4 -> (B, l, N_e)
    ...
    x = nn.LayerNorm()(x)
    x = Dense(cfg.pred_actions, use_bias=False)(x)
    return x
  ...
  def get_state(self, train_cfg: TrainConfig, verbose: bool = False, load_path: str = None, train: bool = True) -> TrainState:
    ...
    s, a, rtg, timestep = jnp.empty((B, l, train_cfg.obs_dim), float), jnp.empty((B, l, 4), int), jnp.empty((B, l), float), jnp.empty((B, l), int)
  ...
  def create_fns(self):
    ...
    def predict(state: TrainState, s, a, rtg, timestep, mask_len: Sequence[int] = None, rng: jax.Array = None, deterministic: bool = False):
      ...
      if mask_len is not None:
        logits = logits[jnp.arange(logits.shape[0]), mask_len-1, :]  # (B, pred_actions)
      action_dim = len(self.cfg.n_actions)
      pred = []
      for pd_a in jnp.array_split(logits, action_dim, -1):
        if deterministic:
